...
public class GestorIORelacion {
    ...
    public static void main(String[] args) {
        try {
            ...
            //--para simular el apagado ...apagar = true;
            System.out.println();
            System.out.print("Apagado <true/false>: " );
            while(Estado)
            {
                if(in.nextBoolean()) Estado = false; 
            }
            //--mandamos a disco las paginas en memoria intermedia
            //========================================================
            System.out.println("===============================================");
            System.out.println("PAGINAS EN EL BUFFER");
            Queue Aux = gmi.getColaLRU();
            int i=0,cola = Aux.size();
            while(i != cola){
                int numero = (Integer)Aux.element();
                Aux.remove();
                IdentificadorPagina ID = new IdentificadorPagina(null,numero);    
                System.out.println("ID: " + ID.getNumero());
                Pagina P = gmi.recuperarPaginaDelBuffer(ID);
                ga.escribirPagina(P,P.getIdentificadorPagina().getNombreArchivo());
                i++;
            }
